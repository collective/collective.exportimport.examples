...
PROJECT_NAME=migration-example-project
STACK_NAME=migration-example-project-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://migration-example-project.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t collective/migration-example-project-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t collective/migration-example-project-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name migration-example-project-frontend-acceptance --link migration-example-project-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/migration-example-project-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name migration-example-project-backend-acceptance -d collective/migration-example-project-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop migration-example-project-frontend-acceptance
	@docker stop migration-example-project-backend-acceptance
